...
{
    public delegate void GetNextPageDelegate();
    ...
    {
        // data source which stores the header(columns) and content(rows) of a table.
        DataTable DataTable { get; }


        // This tells the source to load the table header into the source DataTable
        // The headers could be from the device, or a file and the clould so it's usually asynchronized that we use
        // Task to asynchronously fetch data.
        Task GetHeadersFromSourceAsync();


        // This tells the Source to load the data into the source DataTable
        // This method is called on the UI thread so:
        // It may load it all, but more commonly will load just one batch of rows of a suitable size to optimize performance
        // Returns two indexes into the table, the first row and last row plus 1
        // (Sources should never spend more than 100ms loading, because this will hold up the UI, but should probably do more than one record at a time to get reasonably efficiency)
        // The Task pattern is used becasue we run on the UI thread, and this tells us when the batch is loaded.
        Task<Tuple<int, int>> GetPageFromSourceAsync();


        // This notifies the destination that the table meta-data, incluing the headers (columns), date, tablename has been transferred
        // This is a asynchronous process as the destination might upload the headers to device, cloud etc.
        Task PutHeaderToDestinationAsync();


        // This notifies the destination that the page data(a batch of rows) has been transferred, 
        // so the destination can implement its logic to handle the data.
        // The firstRowIndex and lastRowIndex indicate the cursors where the rows were transferred.
        // For the getNextPage callback function, it enables the destination to trigger a new transaction 
        // to fetch the next batch of rows.
        Task PutPageToDestinationAsync(int firstRowIndex, int lastRowIndex, GetNextPageDelegate getNextPage);

        
        // The ITableDataFlow interface also has a concept of a current row.
        // Either the source or the detination can change the current row.
        // The current row can, for example, select a session in a device or it can be the row that gets deleted by a delete command given to the source or the destination.
